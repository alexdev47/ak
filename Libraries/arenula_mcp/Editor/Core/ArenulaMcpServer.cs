...
public static class ArenulaMcpServer
{
    ...
    public static void StartServer()
    {
        ...
        try
        {
            ...
            _ = Task.Run( async () => { try { await ListenLoop( _cts.Token ); } catch { } } );
    ...
    private static async Task ListenLoop( CancellationToken token )
    {
        while ( !token.IsCancellationRequested && _listener != null && _listener.IsListening )
        {
            try
            {
                ...
                _ = Task.Run( async () => { try { await HandleContext( context ); } catch { } }, token );
            }
            ...
            if ( id != null )
            {
                ...
                var task       = GameTask.RunInThreadAsync( async () =>
                {
                    ...
                    catch ( Exception ex )
                    {
                        ...
                        catch { /* Ignore further dispatch errors */ }
                    }
                    finally
                    {
